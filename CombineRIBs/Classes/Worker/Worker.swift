...
open class Worker: Working {
    ...
    ///
    /// - note: All subscriptions added to the cancellable set provided in the `didStart` method are automatically cancelled
    ...
    private func bind(to interactorScope: InteractorScope) {
        ...
        interactorBindingCancellable = interactorScope.isActiveStream
            .sink { [weak self] (isInteractorActive: Bool) in
...
public extension AnyCancellable {
    ...
    /// Stores the cancellable based on the lifecycle of the given `Worker`. The subscription is cancelled when the
    /// `Worker` is stopped.
    ///
    /// If the given worker is stopped at the time this method is invoked, the subscription is immediately cancelled.
    ///
    ...
    @discardableResult
    func store(in worker: Worker) -> AnyCancellable {
        if var cancellables = worker.cancellables {
            self.store(in: &cancellables)
            worker.cancellables = cancellables
        } else {
            cancel()
            print("Subscription immediately cancelled, since \(worker) is stopped.")
        }
        return self
    }
}
