...
open class Workflow<ActionableItemType> {
    ...
    /// Called when the last step publisher has error.
    ///
    /// Subclasses should override this method if they want to execute logic at this point in the `Workflow` lifecycle.
    /// The default implementation does nothing.
    ...
    public final func subscribe(_ actionableItem: ActionableItemType) -> AnyCancellable {
        guard !compositeCancellables.isEmpty else {
        ...
        return AnyCancellable {
            self.compositeCancellables.forEach { $0.cancel() }
        }
    }
    ...
    /// The set of cancellables that contains all subscriptions including the original workflow
    /// as well as all the forked ones.
    fileprivate var compositeCancellables = Set<AnyCancellable>()

    fileprivate func didCompleteIfNotYet() {
        // Since a workflow may be forked to produce multiple subscribed Combine chains, we should
...
open class Step<WorkflowActionableItemType, ActionableItemType, ValueType> {
    ...
    public final func commit() -> Workflow<WorkflowActionableItemType> {
        ...
        workflow.compositeCancellables.insert(cancellable)
...
public extension AnyCancellable {
    ...
    /// Store the cancellable when the given `Workflow` is cancelled.
    ///
    ...
    ///
    /// - parameter workflow: The workflow to store the cancellable with.
    func store<ActionableItemType>(in workflow: Workflow<ActionableItemType>) {
        workflow.compositeCancellables.insert(self)
    }
    ...
    ///
    /// - parameter workflow: The workflow to store the cancellable with.
    @available(*, deprecated, renamed: "store(in:)")
